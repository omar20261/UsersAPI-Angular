import { Injectable } from '@angular/core';
import { HttpRequest,HttpInterceptor,HttpResponse,HttpHandler,HTTP_INTERCEPTORS,HttpParams,HttpEvent } from '@angular/common/http';
import { Observable, of, throwError } from 'rxjs';
import * as _ from 'lodash';
import * as parse from 'url-parse';

@Injectable({
  providedIn: 'root'
})
export class FakeBackendInterceptor implements HttpInterceptor{
  token:any= 'JWT eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..fF7azKGEc9Oxm4jCRzND83ByQzQjsXtr39lhfGY_Hjk'

  users:any=[
    {
      "_id": 1,
       "Image":`data:image/jpeg;base64,`,
      "name": "Nial Mcgee",
      "jobTitle": "Compensation and Benefits Manager",
    },
    {
      "_id": 2,
      "name": "Mohsin Villanueva",
      "jobTitle": "Elementary School Teacher",
    },
    {
      "_id": 3,
      // Image:'',
      "name": "Kadeem Naylor",
      "jobTitle": "Machinist",
    }
  ]
  constructor() {

  }

  intercept(request: HttpRequest<any>,next: HttpHandler): Observable<HttpEvent<any>> {
    let {pathname,query} = parse(request.url)
        query = this.splitCurrentURL(query)

      if (pathname=='/auth' && request.method === 'POST') {
        return this.Login(request.body)
      }

      else if (pathname=='/api' && request.method === 'GET') {
        return this.API_GET(request.headers,query)
      }

      else if (pathname=='/api' && request.method === 'POST') {
        return this.API_POST(request.body,request.headers)
      }

      else if (pathname=='/api' && request.method === 'PUT') {
        return this.API_PUT(request.body,request.headers)
      }

      else if (pathname=='/api' && request.method === 'DELETE') {
        return this.API_DELETE(request.headers,query)
      }

      return next.handle(request);
   }

   Login(body){
     if(body.username=='Admin'&&body.password=='Admin'){
       return of(new HttpResponse({status:200, body: {success:true,token:this.token} }));
     }
     return of(new HttpResponse({status:406, body: {success:false,msg:'Invalid Username or Password'} }));
   }


   API_GET(headers,query){
     if(headers.get("Authorization")==this.token){
       let Page=parseInt(query.Page),PerPage=parseInt(query.PerPage);
       let list = this.users.slice((Page-1)*7,((Page-1)*7)+PerPage)
       return of(new HttpResponse({status:200, body: {success:true,doc:Object.assign([], list),count:this.users.length} }));
     }
     return of(new HttpResponse({status:406, body: {success:false,msg:' Unauthorised '} }));
   }

   API_POST(body,headers){
     if(headers.get("Authorization")==this.token){
       body._id=this.users.length+1
       this.users.unshift(body);
       return of(new HttpResponse({status:200, body: {success:true,doc:body} }));
     }
     return of(new HttpResponse({status:406, body: {success:false,msg:' Unauthorised '} }));
   }

   API_PUT(body,headers){
     if(headers.get("Authorization")==this.token){
       let ind = _.findIndex(this.users,{_id:body._id})
       this.users.splice(ind,1,body);
       return of(new HttpResponse({status:200, body: {success:true,doc:body} }));
     }
     return of(new HttpResponse({status:406, body: {success:false,msg:' Unauthorised '} }));
   }

   API_DELETE(headers,query){
     if(headers.get("Authorization")==this.token){
       let ind = _.findIndex(this.users,{_id:parseInt(query.id)})
       this.users.splice(ind,1);
       return of(new HttpResponse({status:200, body: {success:true,doc:{}} }));
     }
     return of(new HttpResponse({status:406, body: {success:false,msg:' Unauthorised '} }));
   }


  splitCurrentURL(url){
    if(!url){return {}}
    var url = url.split("?")[1]; // this=true&that=good;
    let params = {}; //init param obj
    url = url.split("&"); // ['this=true','that=good']
    for(var i = 0; i<url.length; i++){
      var split_cache = url[i].split("="); // ['this','true'], ...
    	params[split_cache[0]] = split_cache[1]; // {this:true}, ...
    }
    return params;
  }

}

export let FakeBackendService = {
   provide: HTTP_INTERCEPTORS,
   useClass: FakeBackendInterceptor,
   multi: true
};
